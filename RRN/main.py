...
opt.data_dir = '/mnt/data/SuperRes/lin'
...
def main():
    ...
    train_set = get_training_set(opt.data_dir, opt.scale, opt.data_augmentation, opt.file_list, rgb_type)
    ...
    rrn = RRN(opt.scale, n_c, n_b, rgb_type=rgb_type) # initial filter generate network
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs+1):
        train(train_loader, rrn, opt.scale, criterion, optimizer, epoch, use_gpu, n_c, rgb_type) #fed data into network
        scheduler.step()
        if (epoch) % (opt.snapshots) == 0:
            checkpoint(rrn, epoch)

def train(train_loader, rrn, scale, criterion, optimizer, epoch, use_gpu, n_c, rgb_type=True):
    train_mode = True
    ...
    for iteration, data in enumerate(train_loader):
        ...
        for i in range(T-1):
            if init:
                ...
                init_o = init_temp.repeat(1, scale*scale*chs,1,1)
